...
TEST(binary_indexed_tree, binary_indexed_tree_test)
{
    ...
    {
        ASSERT_EQ(true, bt.query_given_range(i, i) == s_arr[i]);
    }
    int value = 10;
    ...
    FOR_EACH(i, 0, s_arr.size())
    {
        ...
        {
            ASSERT_EQ(true, bt.query_given_range(j - i, j) == temp_sum_func(j - i, j));
        }
    ...
    FOR_EACH(i, 0, s_arr.size())
    {
        s_arr[i] = value;
        bt.update(i, value);
        ASSERT_EQ(true, bt.query_given_range(i, i) == temp_sum_func(i, i));

        ASSERT_EQ(true, bt.query_given_range(0, s_arr.size() - 1) == temp_sum_func(0, s_arr.size() - 1));
